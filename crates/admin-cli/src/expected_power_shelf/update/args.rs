...
///
/// Select the shelf by either BMC MAC address or ID. Supply BMC credentials or a shelf serial
/// number; other update flags must accompany one of those options. With Core PATCH, supplied
/// fields replace their stored values and omitted fields remain unchanged. Supplied labels replace
/// the whole label collection. An empty metadata name or description clears that field.
///
/// Supply BMC username and password together. Core PATCH requires both values to be nonempty;
/// omitting both preserves credentials. Legacy fallback uses the validation rules on the older server.
///
...
/// The legacy shelf update sends a full replacement: omitted values can be cleared, and selecting
/// by ID is rejected. For this path, select by BMC MAC address and supply every value you need to
/// preserve. The legacy request still requires authorization. Other PATCH errors and failed legacy
...
pub(crate) struct Args {
    ...
    #[clap(
        long = "label",
        value_name = "LABEL",
        help = "Replace all metadata labels with the supplied key or key:value entries. Repeat for each label. PATCH preserves omitted labels",
