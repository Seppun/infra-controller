...
and the device's BMC MAC before that. A node no simulated device answers for
is a per-node failure: the batch fails and the node's result says why. The
...
child per node, returned in `jobs` keyed by the caller's node id. Jobs advance
...
Every job completes unless the host names its node in
`FaultConfig::fail_jobs_for_node_ids`, which is set from Rust and not from
configuration. The crate's tests use it to exercise NICo's failure paths. A
